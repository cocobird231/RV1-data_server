...
class TopicRecordNode : virtual public rclcpp::Node
{
private:
	struct HeaderMsgNodes
	{
		...
		MsgNode stamp_type;
		MsgNode stamp;
		MsgNode ref_publish_time_ms;
		MsgNode recvTs;// QoS analyzing
		MsgNode recvFrameID;// QoS analyzing
...
public:
	TopicRecordNode(std::string nodeName) : rclcpp::Node(nodeName)
	{
		this->initF_ = false;
		this->recvFrameID_ = 0;
		...
		this->latestMsgNodePack_["recvTs"] = &this->headerNodes_.recvTs;
		this->latestMsgNodePack_["recvFrameID"] = &this->headerNodes_.recvFrameID;
	}
	...
	void setHeaderNodes(vehicle_interfaces::msg::Header::SharedPtr header, double recvTs)
	{
		...
		this->headerNodes_.recvTs.setContent(recvTs);
		this->headerNodes_.recvFrameID.setContent(this->recvFrameID_++);
		locker.unlock();
	}
...
private:
    ...
    {
		this->setHeaderNodes(std::make_shared<vehicle_interfaces::msg::Header>(msg->header), this->get_clock()->now().seconds());
...
public:
    GroundDetectSubNode(const std::string& nodeName, const std::string& topicName, const std::string& outputDir, SaveQueue<WriteGroundDetectStruct>* saveQue) : TopicRecordSavingNode(nodeName, outputDir), rclcpp::Node(nodeName)
    {
...
public:
    ImageSubNode(const std::string& nodeName, const std::string& topicName, const std::string& outputDir, SaveImgQueue* saveImgQue) : TopicRecordSavingNode(nodeName, outputDir), rclcpp::Node(nodeName)
    {
        addLatestMsgNodePackTag("width", &this->dataNodes.width);
        addLatestMsgNodePackTag("height", &this->dataNodes.height);
        addLatestMsgNodePackTag("filename", &this->dataNodes.filename);
...
public:
    BaseMatSubNode(const std::string& nodeName, const std::string& outputDIR, SaveImgQueue* saveImgQue) : TopicRecordNode(nodeName), rclcpp::Node(nodeName), 
		recvMatInitF_(false), 
		newRecvMatF_(false)
	{
		addLatestMsgNodePackTag("height", &this->dataNodes.height);
...
private:
	void _rgbaCallback(const sensor_msgs::msg::Image::SharedPtr msg)
	{
		...
		this->setHeaderNodes(header, this->get_clock()->now().seconds());
		this->dataNodes.height.setContent(msg->height);
		this->dataNodes.width.setContent(msg->width);
		double timestamp = msg->header.stamp.sec + (double)msg->header.stamp.nanosec / 1000000000.0;
...
public:
	RGBMatSubNode(const std::string& nodeName, const std::string& outputDIR, SaveImgQueue* saveImgQue, const std::string& topicName) : BaseMatSubNode(nodeName, outputDIR, saveImgQue), rclcpp::Node(nodeName)
	{
...
public:
	DepthMatSubNode(const std::string& nodeName, const std::string& outputDIR, SaveImgQueue* saveImgQue, const std::string& topicName) : BaseMatSubNode(nodeName, outputDIR, saveImgQue), rclcpp::Node(nodeName)
	{
		...
		for (const auto& topic : pair.second)// pair: {Timestamp, Topics}
		{
			...
			for (const auto& msg : topic.second)
			{
				...
				if (holder<uint8_t>* val = dynamic_cast<holder<uint8_t>* >(needsHold))
					...
					json[timestamp][topic.first][msg.first] = val->get();
				else if (holder<u_int64_t>* val = dynamic_cast<holder<u_int64_t>* >(needsHold))
					json[timestamp][topic.first][msg.first] = val->get();
